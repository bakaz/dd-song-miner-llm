...
```powershell
cd C:\Users\{username}\Documents\dd-song-miner-llm
py -3.12 -m venv .venv
...
```powershell
python -m dd_song_miner_llm batch-run "D:\{input_dir}" --config config.yaml --work-root "D:\{output_dir}" --result-root "D:\{output_dir}"
```
...
| `json_fix_rounds` | `3` | 当 LLM 返回非 JSON 时，自动让它转换成 JSON 的最大重试轮数。 |
...
| `after_seconds` | `15.0` | 歌曲结束后补多少秒。 |
| `after_next_asr_end_guard_seconds` | `2.0` | after padding 的硬限制：不能超过下一段 ASR segment 的 end 减去此值。 |
| `min_song_seconds` | `30.0` | 低于这个时长的候选会被丢弃。 |
| `merge_gap_seconds` | `35.0` | 同名歌曲间隔小于该值时合并。 |
...
| `video_codec` | `copy` | `copy` 复制原视频流，最快且不降低画质；`auto`、`nv`、`intel`、`amd`、`cpu` 会重编码。 |
...
```powershell
python -m dd_song_miner_llm batch-run "\\{server}\{share}\{input_dir}" `
  --config config.yaml `
  --work-root "D:\song-results" `
...
- `--work-root` 和 `--result-root` 设置为同一目录，避免中间复制。默认为 `runs/batch`。
- 网络目录或中文/日文路径会自动 staging 到 run 目录的 `00_input`，降低 FFmpeg 路径编码风险。
- 输出目录名基于视频文件名，不含时间戳，支持断点续传。

适合 {project_name} 这类结构：
...
$Repo = "C:\Users\{username}\Documents\dd-song-miner-llm"
$InputRoot = "\\{server}\{share}\{input_dir}"
$OutputDir = "D:\song-results"

Set-Location $Repo
& "$Repo\.venv\Scripts\python.exe" -m dd_song_miner_llm batch-run $InputRoot `
  --config "$Repo\config.yaml" `
  --work-root $OutputDir `
  --result-root $OutputDir `
  --marker ".dd_song_miner_done.json" `
  --extensions "mp4,mkv,flv"
```
...
- ASCII 本地路径：直接使用原文件。
- 非 ASCII 路径或 UNC 网络路径（如 `\\{server}\{share}`）：在运行目录 `00_input` 下创建 ASCII 文件名 staging 输入。
...
`copy` 会直接复制原视频流，通常是最快、且画质不变的剪切方式。需要重新编码时，`auto` 会按以下顺序尝试：
...
```powershell
python -m dd_song_miner_llm ffmpeg-info
```
