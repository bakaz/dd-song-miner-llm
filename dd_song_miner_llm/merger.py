...
def _padding_bounds(
    ...
    if song_end_idx + 1 < len(segments):
        after_limit = max(song_end, segments[song_end_idx + 1].end - after_guard)
    else:
...
def build_song_results(
    ...
    min_duration = float(padding_config.get("min_song_seconds", 30.0))
    merge_gap = float(padding_config.get("merge_gap_seconds", 35.0))
