...
def identify_songs(
    ...
    for batch_start, batch in _iter_llm_batches(segments, config):
        ...
        # JSON修复：当返回非JSON时，让LLM转换成JSON
        if not items and content.strip():
            items, content = _fix_json_with_llm(
                client, provider, config, content, batch_debug
            )
...
def _fix_json_with_llm(
    ...
    """当LLM返回非JSON时，让它把内容转换成JSON格式。最多重试3次。"""
    max_rounds = int(config["llm"].get("json_fix_rounds", 3))
    if max_rounds <= 0:
        return [], raw_content

    fix_prompt = f"""下面是之前对ASR歌曲识别任务的回复，但它不是纯JSON格式。
请把其中的歌曲信息提取出来，转换成纯JSON数组。

每个对象只允许包含以下字段：
- title: 歌名
- artist: 歌手（无法判断时填空字符串）
- segment_indices: 属于该歌曲的ASR段落序号数组，按升序排列
- confidence: 0到1的置信度
...
{raw_content}"""
    ...
    for round_num in range(max_rounds):
        try:
            ...
            items = _parse_llm_response(new_content)
            if items:
                return items, new_content

            # 如果新内容非空但解析失败，继续用新内容重试
